...
GLASS_3  = np.asarray([-0.168804, 0.964299, -0.075914, -1.358324, -1.703457, 1.799141, -0.030722])
GLASS_2 = np.asarray([-0.171666, 0.961648, -0.077805, -1.363999, -1.706612, 1.803576, -0.026964])
GLASS_1 = np.asarray([0.227556, 0.940541, 0.122286, -1.404549, -1.463172, 1.321567, -0.011494])
GLASS_4 = np.asarray([-0.178156, 0.571534, -0.072531, -1.423561, -1.658792, 1.844948, -0.373965]) 
GLASS = [GLASS_1, GLASS_3, GLASS_4]
    ...
    for goal in goals:
        ...
        while dist > min_dist and elapsed_time < total_time:
            ...
            if action_interval > 0.005:
                ...
                std = np.std(a_robot, axis=0)
                confidence = np.mean(std) / 0.008
                if confidence > .75:
                    confidence = .75
                ...
                if elapsed_time < 4.5 and human_takeover:
                    ...
                    # confidence = 1.0
                    # qdot_h = 0.01 * qdot_h
                    # qdot_h = np.clip(qdot_h, -0.2, 0.2) + np.clip(noise, -0.075, 0.075)
                    qdot = confidence * 4 * qdot_r + (1 - confidence) * qdot_h
                    if print_flag:
...
def main():
    ...
    cae_model = "models/task_2_cae_dropout_" + str(demos_num)
        ...
        if start:
            while True:
                ...
                if choice == 0:
                    ...
                    dataset = go2goal(conn, GLASS, model)
                    filename = save_loc + "g_drop_" + str(glass_count) + ".pkl"
                    glass_count += 1
                elif choice == 1:
                    print('[*] My goal is notepad')
                    dataset = go2goal(conn, NOTEPAD, model)
                    filename = save_loc + "n_drop_" + str(notepad_count) + ".pkl"
                    notepad_count += 1
