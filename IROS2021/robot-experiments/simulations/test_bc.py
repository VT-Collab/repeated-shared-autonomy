    ...
    for goal in goals:
        ...
        while dist > 0.02 and elapsed_time < total_time:
            ...
            if action_interval > 0.005:
                ...
                qdot_r = model.behavior_clone(state["q"].tolist())
            ...
            if data_interval > 0.1 and elapsed_time >= 2.:            
                data_time = time.time()
                data = [elapsed_time] + [state["q"].tolist()] + [qdot_h.tolist()] + [qdot_r] + [confidence.tolist()]
                dataset.append(data)
...
def main():
    ...
    demos_num = 30
    save_loc = "data/test_runs/" + str(demos_num) + "_demos/"
    cae_model = "models/task_1_cae_bc_state_only_" + str(demos_num)
        ...
        if start:
            ...
            while current_run < total_run:
                ...
                if choice == 0:
                    ...
                    filename = save_loc + "n_bc_" + str(demos_num) + "_" + str(notepad_count) + ".pkl"
                    notepad_count += 1
                elif choice == 1:
                    print('[*] My goal is soupcan')
                    dataset = go2goal(conn, SOUPCAN, model)
                    filename = save_loc + "c_bc_" + str(demos_num) + "_" + str(soupcan_count) + ".pkl"
                    soupcan_count += 1
                elif choice == 2:
                    ...
                    filename = save_loc + "t_bc_" + str(demos_num) + "_" + str(tape_count) + ".pkl"
                    tape_count += 1
