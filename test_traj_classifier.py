...
import torch
import copy
from train_traj_classifier import Net
from train_traj_variation import CAE
...
class Model(object):
    ...
    def encoder(self, c):
        z_mean_tensor, z_var_tensor = self.cae_net.encoder(torch.FloatTensor(c))
        return z_mean_tensor.tolist(), z_var_tensor.tolist()
...
def main():
    ...
    classifier_name = "models/classifier_traj"
    cae_name = "models/vae__traj_1"
    ...
    q_prev_1 = np.asarray([player.x, player.y])
    q_prev_2 = np.asarray([player.x, player.y])
    ...
    while True:
        ...
        c = obs_position + q_prev_2.tolist() + q_prev_1.tolist() + q.tolist()

        # Scale confidence
        confidence = model.classify(c)
        print(confidence)
        ...
        q += 0.01 * np.asarray(a_human)
        # q += 0.05 * (np.asarray(a_robot))# * confidence + np.asarray(a_human) * (1 - confidence))
