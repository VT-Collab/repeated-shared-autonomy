...
torch.save(model, "experts/saved_expert/pg_contcart_new.model")
# model = torch.load("experts/saved_expert/pg_pendulum.model")
# model.load("experts/saved_expert/pg.model")

num_expert = 1000

expert_samples = np.array([model.generate_session(env) for _ in range(num_expert)])
np.save('expert_samples/pg_contcart_new', expert_samples)
