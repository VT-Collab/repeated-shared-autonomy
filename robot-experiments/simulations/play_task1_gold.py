...
class Model(object):
    ...
    def decoder(self, z_with_state):
        action = self.cae_net.dec(torch.FloatTensor(z_with_state))
...
def go2goal(conn, goals, model, z):
    state = readState(conn)
    current_state = np.asarray(state["q"].tolist())
    ...
    for goal in goals:
        ...
        while dist > 0.02 and elapsed_time < total_time:
            ...
            if action_interval > 0.005:
                ...
                qdot_r = model.decoder([z] + state["q"].tolist())
                ...
                if elapsed_time < .5:
                    ...
                    # qdot_h = 0.01 * qdot_h
                    qdot_h = np.clip(qdot_h, -0.1, 0.1) + np.clip(noise, -0.075, 0.075)
                    qdot = confidence * 2. * np.asarray(qdot_r) + (1 - confidence) * qdot_h
                    if print_flag:
                        print("[*] Using robot input only ")
                        print_flag = False
                    # print(qdot_r)
                    # print(max(qdot_h))
                    # qdot = qdot_h
            ...
            if data_interval > 0.1 and elapsed_time >= 2.:            
                data_time = time.time()
                data = [elapsed_time] + [state["q"].tolist()] + [qdot_h.tolist()] + [qdot_r] + [confidence]
                dataset.append(data)
...
def main():
    ...
    cae_model = "models/task_1_gold_" + str(demos_num)
        ...
        if start:
            ...
            while current_run < total_run:
                ...
                if choice == 0:
                    ...
                    z = 1.
                    dataset = go2goal(conn, NOTEPAD, model, z)
                    filename = save_loc + "n_gold_" + str(demos_num) + "_" + str(notepad_count) + ".pkl"
                    notepad_count += 1
                elif choice == 1:
                    print('[*] My goal is soupcan')
                    z = 0.
                    dataset = go2goal(conn, SOUPCAN, model, z)
                    filename = save_loc + "c_gold_" + str(demos_num) + "_" + str(soupcan_count) + ".pkl"
                    soupcan_count += 1
                # elif choice == 2:
                #     print('[*] My goal is coffee cup')
                #     dataset = go2goal(conn, CUP, model)
                #     filename = save_loc + "u_bc_" + str(demos_num) + "_" + str(cup_count) + ".pkl"
                #     cup_count += 1
                elif choice == 2:
                    print('[*] My goal is measuring tape')
                    z = -1.
                    dataset = go2goal(conn, TAPE, model, z)
                    filename = save_loc + "t_gold_" + str(demos_num) + "_" + str(tape_count) + ".pkl"
                    tape_count += 1
