...
def go2goal(conn, goal, model, goal_type):
    ...
    while dist > 0.02 and elapsed_time < total_time:
        ...
        if action_interval > 0.005:
            ...
            if elapsed_time < 2.:
                ...
                # confidence = 1.0
                # qdot_h = 0.01 * qdot_h
                qdot_h = np.clip(qdot_h, -0.05, 0.05) + np.clip(noise, -0.05, 0.05)
        ...
        if data_interval > 0.1 and elapsed_time >= 2.:            
            ...
            data = [elapsed_time] + [state["q"].tolist()] +\
             [qdot_h.tolist()] + [qdot_r.tolist()] + [confidence.tolist()] +\
             [goal_change_time]
            dataset.append(data)
        ...
        if noise_interval > 0.5:
            ...
            # print("goal_change_time: ", goal_change_time)
...
def main():
    ...
    demos_num = 30
    save_loc = "data/test_runs/prior/ours/"
    cae_model = "models/task_1_cae_" + str(demos_num)
        ...
        if start:
            ...
            while current_run < total_run:
                ...
                if choice == 0:
                    ...
                    filename = save_loc + "n_ours_prior_" + str(demos_num) + "_" + str(notepad_count) + ".pkl"
                    notepad_count += 1
                elif choice == 1:
                    print('[*] My goal is soupcan')
                    dataset = go2goal(conn, SOUPCAN, model, "c")
                    filename = save_loc + "c_ours_prior_" + str(demos_num) + "_" + str(soupcan_count) + ".pkl"
                    soupcan_count += 1
                elif choice == 2:
                    print('[*] My goal is measuring tape')
                    dataset = go2goal(conn, TAPE, model, "t")
                    filename = save_loc + "t_ours_prior_" + str(demos_num) + "_" + str(tape_count) + ".pkl"
                    tape_count += 1
