...
demo_trajs = np.load('expert_samples/pg_contcart_new.npy', allow_pickle=True)
...
cost_optimizer = torch.optim.Adam(cost_f.parameters(), 1e-3, weight_decay=1e-5)
...
for i in range(2000):
    ...
    for traj in trajs:
        states, actions, rewards, traj_probs = traj
    ...
    if i % 10 == 0:
        ...
        # plt.show()
        plt.savefig('plots/GCL_learning_curve_new.png')
        plt.close()
