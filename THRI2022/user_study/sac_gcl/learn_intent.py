...
def learn_intent(args):
    ...
    for i_episode in range(1, args.num_eps+1):
        ...
        while not done:
            ...
            if not outOfBounds:
                # print(action)
                next_state, outOfBounds = robot_interface.applyAction(np.concatenate((action, [1, 0, 1])))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--expert', default=2)
    parser.add_argument("--intentFolder", type=int, help="intent folder to read from and save to", default=2)
