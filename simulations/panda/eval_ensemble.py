...
def eval_policy(tasks, max_models, max_runs):
    models = []
    ...
    for task in range(tasks):
        ...
        for run in range(1, max_runs+1):
            savename = "runs/" + "model_ensemble_" + str(tasks) + " _task_" + str(task+1) \
                        + "_run_" + str(run) + ".pkl" 
            env = SimpleEnv(tasks)
            state = env.reset()
            state = state[-1]
            ...
            while not done:
                ...
                xdot_r[:3] = np.mean(actions, axis=0)

                # blending
                # stds.append(max(np.std(actions, axis=0)))
                alpha = 1 - min(max(np.std(actions, axis=0)) / 0.3, 1.)
                alpha = min(alpha, 0.7)
                ...
                if curr_time - start_time >= step_time:
                    ...
                    print("model: {} task:{} alpha: {} run: {} timesteps: {}".format(model_num, task+1, alpha, run, len(data)))
            ...
            demonstration = {}
            demonstration["model"] = "ensemble_tasks_" + str(tasks)
...
def main():
    ...
    for tasks in range(1, max_tasks+1):
        eval_policy(tasks, max_models, max_runs)
