...
def main(model_name, model_num, max_runs):
    ...
    for i in range(model_num):
        cae_model = 'models/' + 'cae_' + "_".join(model_name) + "_" + str(i) + "_old"
        class_model = 'models/' + 'class_' + "_".join(model_name) + "_" + str(i) + "_old"
    ...
    while not rospy.is_shutdown():
        ...
        for task in model_name:
            ...
            for run in range(1, max_runs+1):
                go2home()
                # rospy.sleep(2)            
                filename = "runs/" + "ensemble_model_" + "_".join(model_name)\
                 + "_task_" + task + "_" + str(run) + ".pkl"
                ...
                while not done:
                    ...
                    if s:
                        ...
                        a_robot = np.mean(a_robots, axis=0)
                        alpha = 1 - min(max(np.std(a_robots, axis=0))/0.06, 1.)
                        alpha = min(alpha, 0.6)
                    ...
                    if curr_time - start_time >= steptime:
                        ...
                        print("model: {0} task: {1} run: {2} qdot_h:{3:2.1f} qdot_r:{4:2.1f} alpha:{5:2.1f} reward:{6:2.1f}"\
                            .format("_".join(model_name), task, run, np.linalg.norm(qdot_h), np.linalg.norm(qdot_r), float(alpha),\
                             float(reward)))
                        # print("state: {} xdot: {}".format(s, mover.qdot2xdot(qdot[0])))
                        # print("std: {}".format(max(np.std(a_robots, axis=0))))
                        cum_reward += reward
...
if __name__ == "__main__":
    ...
    for model_name in model_names:
        try:
            r = main(model_name, max_models, max_runs)
            if not r:
                break
            else:
                print(r)
