...
def main():
    ...
    tasknames = ["push1", "push2", "cut1", "cut2", "scoop1", "scoop2",\
                      "open1", "open2"]
    """steptime = 0.4 data"""
    data_dict = pickle.load(open("confidences_deformed_2.pkl", "rb"))
    data_04 = []
    for key in tasknames:
        data_04.append(data_dict[key])
    """steptime = 0.1 data"""
    data_dict = pickle.load(open("confidences_deformed_20.pkl", "rb"))
    data_01 = []
    for key in tasknames:
    ...
    axs[0].set_title("True:Fake::1:2")
    axs[1].set_title("True:Fake::1:20")
    for ax in axs:
